...
from house_epita_dsp_prices.preprocess import process_data, make_encoder, make_scaler
from sklearn.model_selection import train_test_split
from house_epita_dsp_prices.config import CATEGORICAL_COLUMNS, CONTINOUS_COLUMNS, PATH
...
def get_train_data(data: pd.DataFrame) -> (pd.DataFrame, pd.DataFrame):
    X, y = (data.loc[:, data.columns != "SalePrice"], data["SalePrice"])
...
def make_model(
    X_train: pd.DataFrame, y_train: pd.DataFrame, path: str
) -> LinearRegression:
